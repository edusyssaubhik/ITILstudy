...
<!-- Body Starts -->
 ...
 <li>Free 18 PMI approved PDUs with our courses <a href="http://itilstudy.com/ITIL-Training/PMI_PDU.asp">More</a></li>
                   ...
                   <tr>
                    ...
                    <!-- ============== End of Dropdown============ -->
                    <tr id="courses" style="display:block;">
                    <td colspan="8">
                     <table border="0" cellspacing="0" cellpadding="0" >
                    <tr>
                      <td class="TableRowOdd" width="100px"><span class="HeaderLarge">Location</span></td>
                      <td class="TableRowOdd" width="100px"><span class="HeaderLarge">Dates</span></td>
                      <td class="TableRowOdd" width="150px"><span class="HeaderLarge">Timing</span></td>
                      <td class="TableRowOdd" width="140px"><span class="HeaderLarge">Price</span></td>
                      <td class="TableRowOdd" width="100px">&nbsp;</td>
                    </tr>
				  ...
				  %>
                    <tr onMouseOver="javascript:highlightTableRowVersionA(this, '#FFFFCC');">
                      ...
                      <td class="<% = bgColor %>"><a href="venue.asp?courseID=<% = arrAllCourses(0,rowCounter) %>" onClick="return popitup('venue.asp?courseID=<% = arrAllCourses(0,rowCounter) %>')"><% = Trim(arrAllCourses(1,rowCounter)) %><% If Session("CountryOrigin") <> "United Kingdom" Then %><% If arrAllCourses(30,rowCounter) <> "" Then %>, <% = arrAllCourses(30,rowCounter) %><% End If %><% If arrAllCourses(30,rowCounter) = "TX" Then %><sup><font color="red"><b>**</b></font></sup><% End If %><% End If %></a><% If arrAllCourses(30,rowCounter) = "Canada" Then %> (Canada)<% End If %><br></td>
                    
                      <td class="<% = bgColor %>">
					 
					  <% = DAY(arrAllCourses(2,rowCounter)) & " " & MonthName(Month(arrAllCourses(2,rowCounter)),3) & " "  & YEAR(arrAllCourses(2,rowCounter))%>
                        to <br />
                      <% = DAY(arrAllCourses(3,rowCounter)) & " " & MonthName(Month(arrAllCourses(3,rowCounter)),3) & " "  & YEAR(arrAllCourses(3,rowCounter))%>
					
                     
                      </td>
                    
                      <td class="<% = bgColor %>" width="23%"><% = arrAllCourses(4,rowCounter) %>:<% = arrAllCourses(5,rowCounter) %>&nbsp;<% = arrAllCourses(6,rowCounter) %>
                        - <% = arrAllCourses(7,rowCounter) %>:<% = arrAllCourses(8,rowCounter) %>&nbsp;<% = arrAllCourses(9,rowCounter) %>
                      </td>
                      
                      <% If arrAllCourses(28,rowCounter) = "Corporate" Then %>
                      <td class="<% = bgColor %>"><font color="#FE2E2E"><b>Corporate class</b></font></td>
                      <td class="<% = bgColor %>"><div align="center"><font size="3" color="#FE2E2E"><b>---</b></font></div></td>
                      <% Else %>
                      ...
                      <td class="<% = bgColor %>">
					   ...
					   <% If Session("CountryOrigin") <> "United Kingdom" Then %>
                        ...
                        <% = FormatNumber(arrAllCourses(17,rowCounter)) %>
                        <% = Currency_Format_Back %>
                        <sup><font color="red"><b>*</b></font></sup>
                        <% If Session("CountryOrigin") = "India" Then %>
                        +
                        <% = arrAllCourses(11,rowCounter) %>
                        % tax <br />
                        (Total Rs. <%= FormatNumber(arrAllCourses(18,rowCounter)) %> )
                        <% End If 
						...
						<% = FormatNumber(arrAllCourses(17,rowCounter)) %>
						<% = Currency_Format_Back %>
						 + VAT<br>(Total <% = Currency_Format_Front %> <% = FormatNumber(arrAllCourses(18,rowCounter)) %>
						 <% = Currency_Format_Back %>)<sup><font color=""red""><b>*</b></font></sup>
						  <% End If %>
                      </td>
                      <% Else %>
					  ...
					  %>
					   <% = Currency_Format_Front %>
                        <% = FormatNumber(arrAllCourses(10,rowCounter)) %>
                        <% = Currency_Format_Back %>
                        <% If Session("CountryOrigin") = "India" Then %>
                        +
                        <% = arrAllCourses(11,rowCounter) %>
                        % tax <br />
                        (Total Rs. <%= FormatNumber(arrAllCourses(12,rowCounter)) %> )
                        <% End If %>
						 <% ElseIf Session("CountryOrigin") = "United Kingdom" Then
					  %>
                      <% = Currency_Format_Front %>
                        ...
                        VAT <br />
                        (Total <%= FormatNumber(arrAllCourses(12,rowCounter)) %> )
                        <% End If %>
						
                      </td>
                      <% End If %>
                      <% If arrAllCourses(25,rowCounter) <> "Full" Then %>
                      <td class="<% = bgColor %>"><form action="/enrollClass.asp" method="post">
                          <input type="hidden" name="SelCourseID" value="<% = arrAllCourses(0,rowCounter) %>" />
                          <input type="submit" name="Course" value="Enroll" class="ButtonSmall"/>
                        </form></td>
                      <% Else %>
                      <td class="<% = bgColor %>">
                        <img src="images/back/full.gif" />
					   </td>
                      <% End If %>
                    ...
                    <tr>
                      <td colspan="6" class="TableRowEven">&nbsp;</td>
                    </tr>
                    </table>
                    </td>
                    </tr>
