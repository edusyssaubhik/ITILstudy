...
Function Question()
  	...
  	Set tsoObject = fsoObject.OpenTextFile("E:\vhosts\ITILstudy.com\httpdocs\ITIL-CSI-Sample1\sample8Ques\"&QuesNo&".asp")
	'Set tsoObject = fsoObject.OpenTextFile("c:\ITILstudy\ITIL-CSI-Sample1\sample8Ques\"&QuesNo&".asp")
...
Function Updating()
	...
	End If
		...
		ConnObj.Execute "UPDATE CSIsample8QuesResponse SET SelectedChoice = '"&SelectedChoice&"', Marked = '"&Marked&"', Duration = '"&TotalDuration&"' WHERE TestNo='"&TestNo&"' AND QuesNo='"&QuesNo&"'"
...
If Request.Form("go") <> "" Then
	...
	TestNo = Session("TestNo")
 	...
 	Set objRs  = connObj.Execute("SELECT SelectedChoice,Marked FROM  CSIsample8QuesResponse WHERE TestNo = '" & TestNo & "' and QuesNo = '" & QuesNo &"'")
...
If Session("QuesNo") = 0 Then
	...
	For i = 1 To 8
		...
		ConnObj.Execute "INSERT INTO CSIsample8QuesResponse(TestNo, QuesNo, SelectedChoice) VALUES ('"&TestNo&"','"&i&"','')"
    ...
    'Begin Case
		Select Case Action
			...
			Case "Previous"	
				...
				Set objRs = connObj.Execute("SELECT QuesNo, Marked FROM CSIsample8QuesResponse WHERE TestNo = '" & TestNo & "' AND Marked = 'Mark' AND QuesNo < '" & Session("QuesNo") & "'  GROUP BY QuesNo,Marked ORDER BY QuesNo Desc")
				...
				'Retrive The User Skipped Questions
 				Set objRs = connObj.Execute("SELECT QuesNo, SelectedChoice FROM CSIsample8QuesResponse WHERE TestNo = '" & TestNo & "' and SelectedChoice = '' and QuesNo < '" & Session("QuesNo") &"' GROUP BY QuesNo,SelectedChoice ORDER BY QuesNo Desc")
				...
				End if
					...
					Set objRs = ConnObj.Execute("Select SelectedChoice, Marked, Duration from CSIsample8QuesResponse WHERE TestNo='"&TestNo&"' AND QuesNo='"&QuesNo&"'")
			...
			Case "Next"
				 ...
				 End If
					...
					If ShowMarked = "Yes" Then
						...
						'Retrive The User Maked Questions'
						Set objRs = connObj.Execute("SELECT QuesNo, Marked FROM CSIsample8QuesResponse WHERE TestNo = '" & TestNo & "' AND Marked = 'Mark' AND QuesNo > '" & Session("QuesNo") & "'  GROUP BY QuesNo,Marked ORDER BY QuesNo")
					...
					ElseIf ShowSkipped = "Yes" Then
				    	...
				    	'Retrive The User Skipped Questions'
						Set objRs = connObj.Execute("SELECT QuesNo, SelectedChoice FROM CSIsample8QuesResponse WHERE TestNo = '" & TestNo & "' and SelectedChoice = '' and QuesNo > '" & Session("QuesNo") &"' GROUP BY QuesNo,SelectedChoice ORDER BY QuesNo")
			...
			Case "Mark"
				...
				Else
					...
					ConnObj.Execute "UPDATE CSIsample8QuesResponse SET SelectedChoice = '"&SelectedChoice&"', Marked = '"&Marked&"' WHERE TestNo='"&TestNo&"' AND QuesNo='"&QuesNo&"'"
			...
			Case "Show Marked"
				...
				Set objRs = connObj.Execute("SELECT TOP 1 QuesNo,SelectedChoice,Marked FROM CSIsample8QuesResponse WHERE TestNo = '" & TestNo & "' AND Marked = 'mark' GROUP BY QuesNo,selectedChoice,Marked ORDER BY QuesNo")
			...
			Case "Show Skipped"
				...
				'Else Show View All Answers'
				Set objRs = connObj.Execute("SELECT TOP 1 QuesNo FROM CSIsample8QuesResponse WHERE TestNo = '" & TestNo & "' and selectedChoice = ''  GROUP BY QuesNo ORDER BY QuesNo")
		...
		Case "View All Answers"
				...
				Set objRs = ConnObj.Execute("SELECT  QuesNo, SelectedChoice, Marked  from CSIsample8QuesResponse WHERE TestNo='"&TestNo&"' ORDER BY GID")
				Array1 = objRs.GetRows
