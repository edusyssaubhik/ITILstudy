...
<% 
	...
	Else
%>
<!--#include virtual="/includes/conn_ng.asp"-->
<%
'Written By : Bharath'
'Created Date: 25/02/2011'
'Modified By: Bharath'
...
'strCourseIds = "SELECT courseid, city, CourseDate, enddate FROM course WHERE country = 'US' AND status <> 'Cancelled' AND status <> 'Completed' ORDER BY CourseDate"

strCourseIds = "SELECT course.courseid, course.city, course.CourseDate, city.state_code, course.instructor FROM course course, city city WHERE course.city = city.city And "& strCountry &" AND course.status <> 'Cancelled' AND course.status <> 'Completed' And course.CourseDate >= '"&Now() - 1&"' order by course.CourseDate, course.city asc"
...
<table width="600px" border="1" class="table1" >
  ...
  <% Do Until objRs.EOF
	 ...
	 If (CDate(CourseDate)-(Date()-1)>=0) Then
		 ...
		 strEnrollments = "SELECT count(*) AS Total FROM enrolledusers WHERE courseid = '"& CourseId &"' AND status <> 'Cancel' AND status <> 'Re-Schedule' And (us_status Is Null OR us_status <> 'Dummy')"
		 ...
		 strPaid = "SELECT count(*) AS Paid FROM enrolledusers WHERE checkreceived = 'Yes' AND courseid = '"& CourseId &"' AND status <> 'Cancel' AND status <> 'Re-Schedule'"
		 ...
		 strPayment = "SELECT us_status FROM enrolledusers WHERE (checkreceived <> 'Yes' OR checkreceived Is Null) AND courseid = '"& CourseId &"' AND status <> 'Cancel' AND status <> 'Re-Schedule'"
