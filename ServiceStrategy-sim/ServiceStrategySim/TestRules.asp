...
If  TestNo = "" Then
	...
	ConnObj.Execute "INSERT INTO SimStrategyUsertable(EMail, LastName, FirstName) VALUES ('"&PayerEmail&"', '"&LastName&"','"&FirstName&"')"	
	...
	rsQues.Open "SELECT TestNo from SimStrategyUsertable WHERE EMail='"&PayerEmail&"' order by TestNo Desc"
...
Else
			...
			'Retrive The Maximum Question Number Increment & Display That Question Continue The Test'
				Set objRs = connObj.Execute("SELECT Max(QuesNo) as QuesNo FROM SimStrategyTestResponse WHERE Duration <> '' And TestNo = '"& Session("TestNo") &"'")
