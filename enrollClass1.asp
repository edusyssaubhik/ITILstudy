...
<!-- Body Starts -->
	...
	'Currency Format Display Functions'
	 ...
	 If SelectedCourseID <> "" Then
		 strQuery = "SELECT * FROM itil_course WHERE country = (SELECT country FROM itil_course WHERE courseid = '"&SelectedCourseID&"') AND startdate BETWEEN '"&Now() - 1&"' AND DATEADD(day,180,'"&Now()&"') AND status <> 'Cancelled' order by startdate,city desc"
		 Else
		 strQuery = "SELECT * FROM itil_course WHERE country = '" & Session("CountryOrigin") & "' AND startdate BETWEEN '"&Now() - 1&"' AND DATEADD(day,180,'"&Now()&"') AND status <> 'Cancelled' order by startdate,city desc"
		 End If
 ...
 <tr>
  <td height="235" align="left" valign="top" bgcolor="#FFFFFF">
   <table width="100%" border="0" cellspacing="0" cellpadding="8">
	...
	<h1 class="PageTitle">ITILstudy Foundation Classroom Program
    ...
    <table width="97%" border="0" cellpadding="0" cellspacing="0" class="general-body">
      <form action="/enrollsuccess.asp" method="post" onsubmit="return validate_form(this)">
        <tr>
          <td><div style="margin-left:-0;"><span class="Required">
              ...
              <table border="0" cellpadding="5" cellspacing="0" width="100%">
                ...
                <tr>
                    ...
                    <tr>
                      <td><span class="general-bodyBold">Street:</span></td>
                      <td><input type="text" name="street" id="street" value="<% = Session("street1") %>" ></td>
                      </tr>
                       <tr>
                      <td><span class="general-bodyBold">City/Town:</span></td>
                      <td><input type="text" name="city" id="city"  value="<% = Session("city") %>"  ></td>
					  
                      <td><span class="general-bodyBold"> <% If Session("CountryOrigin")="United Kingdom"  Then%>County<%Else%>State<%End If%>:</span></td>
                      <td><input type="text" name="state1" id="state1"  value="<% = Session("state1") %>" ></td>
                      </tr>
                       <tr>
                      <td><span class="general-bodyBold">PostalCode:</span></td>                  
                      <td><input type="text" name="postalCode" id="postalCode" value="<% = Session("postalCode") %>"></td>
                      </tr>
					   <tr>
				<td colspan="4" ><span class="Note">*The above address and contact number provided would be used to register exam and certificates would be dispatched to the same</span></td></tr>
                <tr>
