<!--#include virtual="/includes/header.asp"-->
<!--#include virtual="/includes/innerwhyusbanner.html"-->
<!--#include virtual="/metatags/courseBenefits_metatag.html"-->
<!-- Body Starts -->

<div>
  <table width="930" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      <td height="235" align="left" valign="top" bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="0" cellpadding="8">
          <tr>
            <td width="2%" class="Header">&nbsp;</td>
            <td width="25%" class="PageTitle">&nbsp;</td>
            <td width="73%" class="breadcrumb"><a href="/index.asp" title="Home">Home</a> &raquo; FREE RESOURCES &raquo; Free Study Guide and Podcast  </td>
          </tr>
          <tr>
            <!--#include virtual="/includes/innerLeftMenu.asp"-->
            <td width="73%" rowspan="4" background="/images/back/left_line.jpg" class="general-body"><div style="margin-top:-12px; width:97%;">
              <!-- Content Start From Here-->
               <h1 class="PageTitle">FREE SAMPLE GUIDE AND PODCAST - FOR ITIL<Sup>&reg;</sup> FOUNDATION CERTIFICATION EXAM COURSE</h1>
              <p class="Header">Free Sample Guide and Podcast on the "Introduction" Chapter from ITIL v3 Foundation</p>
               <ul>
                <li>ITILstudy Guides are available for the <a href="http://www.itilstudy.com/course-syllabus.asp"/>overall syllabus for ITIL Foundation syllabus</a>. These are available to students who <a href="http://www.itilstudy.com/ITIL-Online-Courses.asp"/>enrol in relevant ITILstudy course</a>.</li>
                <li>The guides in ITILstudy familiarize students about the concepts they should know about the specific Lifecycle phases.</li>
                </ul>
                <span class="SubHeader">Sample guides available in ITILstudy, for example - "Introduction to ITIL Foundation" chapter provides you an idea of our study materials.</span>
                <ul>
                  <li><A href="http://www.APMGstudy.com/ITILdocs/Foundation/guides/1.Introduction.pdf" target="_blank">Download Presentation - Study Guide</a> </li>
                  <li><b>Optional</b> : <span class="SubHeader">Podcasts (audio/visual presentations) :</span> ITILstudy Podcasts are included with all our guides, and allow students to listen to our audio/visual presentations. These podcasts can be copied to your iPhone / iPod / iPAD / Android phone / tab and you can listen to Podcasts when driving to work, traveling or resting. <br><br>
                 <a href="/ITIldocs/podcasthelp.pdf" target="_blank">Please click here for help on how to download the Podcasts</a>.<br><br>
                  To subscribe, copy the following links to iTunes:<br>
                <span style="color:#0075D0;">http://www.APMGstudy.com/ITILdocs/Foundation/Podcast/1.Introduction.rss</span><br><br>
                  Please note: Podcasts will only be available to students who <% If Session("CountryOrigin") = "United Kingdom" Then %>enrol<% Else %>enroll <% End If %> in our courses and have access to ITILstudy Guides.
                  </li>
                  </ul>


              

             <!-- Content End From Here-->
              </div></td>
          </tr>
        </table></td>
    </tr>
  </table>
</div>
<!-- Body Ends -->
<!--#include virtual="/includes/footer.html"-->
