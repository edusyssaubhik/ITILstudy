...
Function Question()
	...
	Set tsoObject = fsoObject.OpenTextFile("E:\vhosts\ITILstudy.com\httpdocs\ITILsim\sim1-V2\"&QuesNo&".asp")
	'Set tsoObject = fsoObject.OpenTextFile("c:\ITILstudy\ITILsim\sim1-V2\"&QuesNo&".asp")
...
Function Updating()
	...
	End If
		...
		ConnObj.Execute "UPDATE FoundV2_Sim1TestResponse SET SelectedChoice = '"& SelectedChoice &"', Marked = '"& Marked &"', Duration = '" & TotalDuration &"' WHERE TestNo='"& TestNo &"' AND QuesNo='"& QuesNo &"'"
...
If Request.Form("go") <> "" Then
	...
	Session("QuesNo") = QuesNo
 	...
 	Set objRs  = connObj.Execute("SELECT SelectedChoice,Marked FROM  FoundV2_Sim1TestResponse WHERE TestNo = '"& TestNo &"' and QuesNo = '"& QuesNo &"'")
...
ElseIf Session("ICQuesNo") <> "" Then
	...
	Set objRs = ConnObj.Execute("Select SelectedChoice, Marked from FoundV2_Sim1TestResponse WHERE TestNo='"& TestNo &"' AND QuesNo='"&   Session("ICQuesNo") &"'")
	 ...
	 'Begin Case
		Select Case Action
			...
			Case "Previous"	
				...
				Set objRs = connObj.Execute("SELECT QuesNo, Marked FROM FoundV2_Sim1TestResponse WHERE TestNo = '"& TestNo &"' AND Marked = 'Mark' AND QuesNo < '"& Session("QuesNo") &"'  GROUP BY QuesNo,Marked ORDER BY QuesNo Desc")
        ...
        'Retrive The User Skipped Questions'
				...
				'Retrive The User Skipped Questions
 				Set objRs = connObj.Execute("SELECT QuesNo, SelectedChoice FROM FoundV2_Sim1TestResponse WHERE TestNo = '"& TestNo &"' and SelectedChoice = '' and QuesNo < '"& Session("QuesNo") &"' GROUP BY QuesNo,SelectedChoice ORDER BY QuesNo Desc")
				...
				'Retrive The SelectedChoice, Marked, Duration Based On Question Number And Test Number'
					...
					Set objRs = ConnObj.Execute("Select SelectedChoice, Marked, Duration from FoundV2_Sim1TestResponse WHERE TestNo='"& TestNo &"' AND QuesNo='"& QuesNo &"'")
			...
			Case "Next"
				 ...
				 End If
					...
					If ShowMarked = "Yes" Then
						...
						'Retrive The User Maked Questions'
						Set objRs = connObj.Execute("SELECT QuesNo, Marked FROM FoundV2_Sim1TestResponse WHERE TestNo = '"& TestNo &"' AND Marked = 'Mark' AND QuesNo > '"& Session("QuesNo") &"'  GROUP BY QuesNo,Marked ORDER BY QuesNo")
					...
					ElseIf ShowSkipped = "Yes" Then
				    	...
				    	'Retrive The User Skipped Questions'
						Set objRs = connObj.Execute("SELECT QuesNo, SelectedChoice FROM FoundV2_Sim1TestResponse WHERE TestNo = '"& TestNo &"' and SelectedChoice = '' and QuesNo > '"& Session("QuesNo") &"' GROUP BY QuesNo,SelectedChoice ORDER BY QuesNo")
			...
			Case "Mark"
				...
				Else
					...
					ConnObj.Execute "UPDATE FoundV2_Sim1TestResponse SET SelectedChoice = '"& SelectedChoice &"', Marked = '"& Marked &"' WHERE TestNo='"& TestNo &"' AND QuesNo='"& QuesNo &"'"
			...
			Case "Show Marked"
				...
				Set objRs = connObj.Execute("SELECT TOP 1 QuesNo,SelectedChoice,Marked FROM FoundV2_Sim1TestResponse WHERE TestNo = '"& TestNo &"' AND Marked = 'mark' GROUP BY QuesNo,selectedChoice,Marked ORDER BY QuesNo")
			...
			Case "Show Skipped"
				...
				'Else Show View All Answers'
				Set objRs = connObj.Execute("SELECT TOP 1 QuesNo FROM FoundV2_Sim1TestResponse WHERE TestNo = '"& TestNo &"' and selectedChoice = ''  GROUP BY QuesNo ORDER BY QuesNo")
		...
		Case "View All Answers"
				...
				Set objRs = ConnObj.Execute("SELECT  QuesNo, SelectedChoice, Marked  from FoundV2_Sim1TestResponse WHERE TestNo='"& TestNo &"' ORDER BY GID")
				Array1 = objRs.GetRows
