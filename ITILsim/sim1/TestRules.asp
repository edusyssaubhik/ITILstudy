...
If  TestNo = "" Then
	...
	ConnObj.Execute "INSERT INTO SimITILusertable(EMail, LastName, FirstName) VALUES ('"&PayerEmail&"', '"&LastName&"','"&FirstName&"')"	
	...
	rsQues.Open "SELECT TestNo from SimITILusertable WHERE EMail='"&PayerEmail&"' order by TestNo Desc"
...
Else
			...
			'Retrive The Maximum Question Number Increment & Display That Question Continue The Test'
				Set objRs = connObj.Execute("SELECT Max(QuesNo) as QuesNo FROM SimITILTestResponse WHERE Duration <> '' And TestNo = '"& Session("TestNo") &"'")
