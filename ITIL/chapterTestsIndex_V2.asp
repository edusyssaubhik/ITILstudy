...
rsQues.Open "select KNo, Areatitle from KArea_ITIL"%>
                <div>
                  <table width="95%" border="0" cellspacing="3" cellpadding="3" align="left" class="general-body">
                    <form method="post" action="/ITIL/quessget_V2.asp" target="_blank">
                      <%Dim KNumber, count
...
%>
					  ...
					  strQuery ="SELECT COUNT(*) as QuestionNo FROM Question_ITILFondDB_Dec2011  WHERE KNo = '" & Session("KNO") &"' "
                   ...
                   'response.Write(questionNo)
                      ...
                      <tr>
                        ...
                        <td class="general-body"><select name="NoofQues" class="TeXtFielddropdown" style="width:45px;">
                            <option <% If 5 <= questionNo Then %> selected="selected"<% Else %>  disabled="disabled" <% End If %> VALUE="5">5</option>
                            <option <% If 10 <= questionNo Then %> selected="selected"<% Else %>  disabled="disabled" <% End If %> VALUE="10">10</option>
                            <option <% If 15 <= questionNo Then %> selected="selected"<% Else %>  disabled="disabled" <% End If %> VALUE="15">15</option>
                            <option <% If 20 <= questionNo Then %> selected="selected"<% Else %>  disabled="disabled" <% End If %> VALUE="20">20</option>
                            <option <% If 25  <= questionNo Then %> selected="selected"<% Else %>  disabled="disabled" <% End If %> VALUE="25">25</option>
                          </select></td>
                      </tr>
                      ...
                      <tr>
                        <td COLSPAN="2" class="general-body"><% If Session("State") = "verified1" OR Session("State") = "verified2" OR Session("State") = "verified3" OR Session("State") = "verified5" OR Session("State") = "verified6" OR Session("State") = "verified7" OR Session("State") = "verified4" OR Session("State") = "verified13" Then	%>
