 ...
 <% 
     ...
     If (CInt(Rs("courseid")) <> CInt(rqCourseId)) Then 
	  ...
	  <option value="<% = Rs("courseid") %>">
	   ...
	   <% = Rs("city") %> : <% = FormatDateTime(Rs("startdate"),1) %> to <% = FormatDateTime(Rs("enddate"),1) %> for Rs.<% If (CDate(Rs("startdate"))-(Date()+Rs("applicabledays")+1) >= 0) Then %><% = FormatNumber(Rs("afterEBdiscountwithtax")) %><% Else %><% = FormatNumber(Rs("pricewithtax")) %><% End If %>
	  <!--
       <option value="<% = Rs("city") %> : <% = FormatDateTime(Rs("startdate"),1) %> to <% = FormatDateTime(Rs("enddate"),1) %> for Rs.<% If (CDate(Rs("startdate"))-(Date()+Rs("applicabledays")+1) >= 0) Then %><% = FormatNumber(Rs("afterEBdiscountwithtax")) %><% Else %><% = FormatNumber(Rs("pricewithtax")) %><% End If %> ? <% = Rs("courseid") %>">
