...
'Declare The Variables'
 ...
 Dim message
 Dim courseDate
 Dim strQuery,Rs1,Rs2,OnlineCourseId,rs3,Rs4,Rs5,Rs6
 ...
 Dim strQuery5,rqOnlineEnrollUserId,maxRevLevel
 Dim instructorFirstRow,instructorLastRow,arrAllInstructors
 Dim rqOnlineCourseId,instructorRowCounter,instructorNumRows
...
If Request.Form("OnlineCourseId") <> "" then

rqOnlineCourseId = Request.Form("OnlineCourseId")

 Else
 
rqOnlineCourseId = Request.Querystring("OnlineCourseId")
...
If rqCourse = "pastcourses" Then 
	...
	strQuery = "SELECT ID,address_city, date_entered, date_valid, payer_status, item_name from PayPalDbTest WHERE address_country = '"& session("country") &"' And item_name='"& session("brand") & "' AND  date_entered < '"& Now() &"' order by date_entered desc"

    response.Write strQueryf

ElseIf (rqCourse = "futurecourses" OR rqCourse = "") Then 

	strQuery = "SELECT ID,address_city, date_entered, date_valid, payer_status, item_name from PayPalDbTest WHERE address_country = '"& session("country") &"' And item_name='"& session("brand") & "' AND  date_entered >= '"& Now() &"' order by date_entered asc"
...
'If Click one course in course page that particular course will display here'
 ...
 strQuery1 = "SELECT * FROM itil_course WHERE CourseId = '"& rqOnlineCourseId &"' And courseType='"& session("brand") &"'"

 Rs1.Open strQuery1,Conn

'Sql Query For Retrive The enrolledusers particular course'

  strQuery2 = "SELECT * FROM PayPalDbTest WHERE Id = '"& rqOnlineCourseId &"' order by Id asc"
...
<script language ="javascript">
    ...
    function Form_Validator(theform) {
        ...
        if (theform.item_name.selectedIndex == "") {
    ...
    function openit(theform) {
        ...
        var enrolledUserId = theform.OnlineEnrollUserId.value
  ...
  <table border="1" width="1470px" class="dbborder" cellspacing="0">
    <tr>
      <td colspan="3" height="35"><div align="center"><font color="#00B0EC" face="Arial" size="4">Welcome to <span style="color:#FF3300">
          <% = Session("country") %>
          </span> scrumstudy Back Office</font></div></td>
    </tr>
    <tr>
...
If session("brand") = "" Then %>  
    ...
    <form action="OnlineCourses.asp" name="price" method="post" onsubmit="return validate_form(this)">
   <table  width="100%" cellpadding="3" cellspacing="5">
     <tr align="center"><td >
      <b>Select the type of course</b>:
                    		<select name="brand" id="brand" value="brand">
                            <option value="0" selected="selected">-- Select Course --</option>
                            <option value="Understand ITIL Exam">Understand ITIL Examr</option>
                            <option value="Free Simulated Test">Free Simulated Test</option>
                            <option value="ITILstudy Blend Course - 1 Months Course">ITILstudy Blend Course - 1 Months Course</option>
                            <option value="ITILstudy Blend Course - 3 Months Course">ITILstudy Blend Course - 3 Months Course</option>
                            <option value="ITILstudy ITIL Foundation Pre-Prep Course">ITILstudy ITIL Foundation Pre-Prep Course</option>
                            <option value="ITILstuyd Blend Course">ITILstuyd Blend Course</option>
                            <option value="Preparatory Program for ITILstudy Classroom Traning">Preparatory Program for ITILstudy Classroom Traning</option>
                            <option value="ITILstudy Service Strategy Online Course - 3 Months Course">ITILstudy Service Strategy Online Course - 3 Months Course</option>
                            <option value="ITILstudy Online Course">ITILstudy Online Course</option>
                            <option value="Understand ITIL Exam">Understand ITIL Exam</option>
                            <option value="Free 40 Questions Simulated Test">Free 40 Questions Simulated Test</option>
                            <option value="Preparatory Program for ITILstudy Classroom Traning">PROJstudyFoundation Pre-Prep for Classroom Pre-Course Course which is a one Month Course.</option>
  ...
  <% elseIf session("brand") <> "" Then %>

    
    <tr>
      <td  height="25px" ><span id="htMap"><% If Not Session("typeofuser") = "CSR" Then %><a href="OnlineCourses.asp?type=add&brand=<% =rqBrand%>">Add Course</a> | <% End If %> <a href="OnlineCourses.asp?Course=pastcourses&brand=<% =rqBrand%>">Past Courses</a> | <a href="OnlineCourses.asp?Course=futurecourses&brand=<% =rqBrand%>">Future Courses</a> |<!-- <a href="ExamBooking.asp">Exam Booking</a> |--></span><a href="OnlineCourses.asp?type=selectcourse"  style="color:#F00; ! important; font-size:14px; ! important;">Select Brand</a></td>
    ...
    <% If rqType = "add" Then %>
    <tr>
      <td align="left" height="200px" colspan="2"><form name="addcourse" action="addEditDeleteCourse.asp" method="post" onsubmit="return Form_Validator(this)">
      <input type="hidden" name="brand" value="<% =session("brand") %>">
...
'And Display That Particular Details'
    ...
    <td  height="200px"><form name="addcourse" action="addEditDeleteCourse.asp" method="post" onsubmit="return Form_Validator1(this)">
        ...
        <tr>
          ...
          <td>&nbsp;&nbsp;
            <select name="item_name" id="item_name">
              <option>--Select--</option>
              <option <% If Rs4("item_name") = "Normal" Then %> selected="selected" <% End If %> value="Normal">Normal</option>
              <option <% If Rs4("item_name") = "Corporate" Then %> selected="selected" <% End If %> value="Corporate">Corporate</option>
               <option <% If Rs4("item_name") = "Weekend" Then %> selected="selected" <% End If %> value="Weekend">Weekend</option>
                  
            </select>
          </td>
        </tr>          
          <tr>
            <input type="hidden" name="editcourseId" value="<% = Rs4("OnlineCourseId") %>">
            <td>City:</td>
            <td>&nbsp;&nbsp;
 ...
 %>
  ...
  <td height="3765" valign="top"><table border="0" style="margin-left:-4px;margin-top:-4px;">
      <tr>
        <td><table border="1" cellpadding="5" cellspacing="0">
         <form name="course" action="OnlineCourses.asp?type=submit" method="post">
         
        <tr><td><b>Online Courses:</b></td>
 			<td colspan="5"><select name="OnlineCourseId" id="Onlinecourses" style="width:350px">
             <option value="">-- Select --</option>
             <% If IsArray(arrAllRecords) Then
          		 For rowcounter = firstRow to allNumRows 
				OnlineCourseId = arrAllRecords(0,rowcounter) 
				
			  %>
 				<option value="<% = OnlineCourseId %>"><% = arrAllRecords(1,rowcounter) %>&nbsp;<% = FormatDateTime(arrAllRecords(2,rowcounter),1) %>&nbsp;to&nbsp;<% = (arrAllRecords(3,rowcounter)) %></option>
         <% Next %>
  ...
  %>
            <tr id="htMap">
              <td>
              <% If (arrAllRecords(5,rowcounter) = "Understand ITIL Exam") Then %>
               Understand ITIL Exam         
              <% ElseIf (arrAllRecords(5,rowcounter) = "ITILstudy Online Course") Then %>
               ITILstudy Online Course
              <% Elseif (arrAllRecords(5,rowcounter) = "ITILstudy Blend Course - 3 Months Course") Then %>
               ITILstudy Blend Course - 3 Months Course
              <% End If %>
              </td>
              <% OnlineCourseId = arrAllRecords(0,rowcounter) %>
              <td><% = arrAllRecords(1,rowcounter) %></td>
              <td><a href="OnlineCourses.asp?OnlineCourseId=<% = OnlineCourseId %>&Course=<% = rqCourse %>&page=<% = iPageCurrent %>">
                <% = FormatDateTime(arrAllRecords(2,rowcounter),1) %>
                to
                <% = (arrAllRecords(3,rowcounter)) %>
                </a></td>
              <% If Not Session("typeofuser") = "CSR" Then %>
              <td><a href="OnlineCourses.asp?Course=<% = rqCourse %>&type=Edit&editId=<% = OnlineCourseId %>">Edit</a></td>
              <td><a href="addEditDeleteCourse.asp?delete=<% = OnlineCourseId %>" onclick="return DeleteAlert(this.href);">Delete</a></td>
              <td><b>
            ...
            <tr id="htMap">
              <td colspan="8"><!--#include file="pagination.asp"-->
               <% 
			    ps = getPaginationString(page, (numRows + 1), 10, 2, "OnlineCourses.asp?Course="& rqCourse &"")
				  Response.Write(ps)
	           %>
 ...
 %>
      ...
      <td><table  width="1800px" border="1" valign="top" cellspacing="0">
          <tr>
            ...
            <td bgcolor="#A7DBFB" align="center" width="100px"><b>PI Details</b></td>
            <td bgcolor="#A7DBFB" align="center" width="100px"><b>Date Of Enrollment</b></td>
       ...
       %>
          <form name="checkreceived" action="OnlineEnrolledDb.asp?type=review" method="post" onsubmit="return Form_Validator2(this)">
            <tr id="htMap">
              ...
              <!--------------------------------------------- First Name -------------------------------------------------------------->
              <td align="center"><% = arrAllQueries(8,enrollrowcounter) %></td>
              <input type="hidden" name="firstname" value="<% = arrAllQueries(8,enrollrowcounter) %>">

              <!--------------------------------------------- Last Name -------------------------------------------------------------->

              <td align="center"><% = arrAllQueries(9,enrollrowcounter) %></td>
              <input type="hidden" name="lastname" value="<% = arrAllQueries(9,enrollrowcounter) %>">

              <!--------------------------------------------- Email Id -------------------------------------------------------------->

              <td align="center"><% = arrAllQueries(7,enrollrowcounter) %><br /><br />
              <input type="hidden" name="email" value="<% = arrAllQueries(7,enrollrowcounter) %>">

              <!--------------------------------------------- Name Of The Employer-------------------------------------------------------------->

              <td align="center"><% 'Employer Name not in the PayPalDbTest table%></td>
              <input type="hidden" name="employeer" value="<% %>">

              <!--------------------------------------------- Phone NO -------------------------------------------------------------->
              <td align="center"><% = arrAllQueries(59,enrollrowcounter) %></td>
              <input type="hidden" name="phoneno" value="<% = arrAllQueries(59,enrollrowcounter) %>">

              <!--------------------------------------------- Address -------------------------------------------------------------->
			   <td align="center"><% = arrAllQueries(32,enrollrowcounter) %></td>              
              <input type="hidden" name="CourseDetails" value="<% =  arrAllQueries(32,enrollrowcounter) %>" />
              
              <!--------------------------------------------- Course details -------------------------------------------------------------->

              <td align="center"><% = arrAllQueries(2,enrollrowcounter) %><br /><% = arrAllQueries(39,enrollrowcounter) %>&nbsp; to&nbsp; <% = arrAllQueries(40,enrollrowcounter) %></td>
              <%  courseDate = arrAllQueries(39,enrollrowcounter) %>


              <!--------------------------------------------- Amount-------------------------------------------------------------->
			   <td align="center"><% = arrAllQueries(18,enrollrowcounter) %></td>
               <input type="hidden" name="Amount" value="<% = arrAllQueries(18,enrollrowcounter) %>" />

	         <!--------------------------------------------- PI Details-------------------------------------------------------------->
			   
              <td><a href="proformaInvoiceDetails.asp?OnlineEnrollUserId=<% = arrAllQueries(0,enrollrowcounter) %>">Invoice</a></td>

              <!---------------------------------------------Date Of Enrollment-------------------------------------------------------------->
              <td align="center"><% = FormatDateTime(arrAllQueries(39,enrollrowcounter),1) %></td>
                	...
                	<td>
                     ...
                     <tr>
                      <td><% if  (arrAllQueries(11,enrollrowcounter)  = "Yes") then%>
                        ...
                        <% End if %>
                        <% If arrAllQueries(62,enrollrowcounter) = 1 Then 
                      Response.Write("Check received email sent") %>
                        <a href="javascript:void(0)" onClick="window.open('paymentreceivedemail.asp?OnlineEnrollUserId=<% = arrAllQueries(0,enrollrowcounter) %>')"><BR />Check recd Email</a>
                        <% Else %>
                        <a href="javascript:void(0)" onClick="window.open('paymentreceivedemail.asp?OnlineEnrollUserId=<% = arrAllQueries(0,enrollrowcounter) %>')"><BR />Check recd Email</a>
                        <% End if %>
                       ...
                       <input type="hidden" name="OnlineEnrollUserId" value="<%= arrAllQueries(0,enrollrowcounter) %>" />
                        ...
                        <a href="javascript:void(0)" onClick="window.open('sendmails_Venue.asp?OnlineCourseId=<% = OnlineCourseId %>&OnlineEnrollUserId=<%= arrAllQueries(0,enrollrowcounter) %>')"><BR />Send Venue Detail</a>
                         <% End If %> 
                       <% If  (arrAllQueries(61,enrollrowcounter) = 1) then %>
                        <br /><br />
          ...
          <form name="form1" action="OnlineReScheduledCourses.asp" method="post" onsubmit="return Form_Validator5(this)">
              ...
              <input type="hidden" name="OnlineCourseId" value="<% = arrAllQueries(20,enrollrowcounter) %>">
              ...
              <input type="hidden" name="modeofpayment" value="<% = arrAllQueries(32,enrollrowcounter) %>" />
              <input type="hidden" name="PaidAmount" value="<% = arrAllQueries(8,enrollrowcounter) %>" />
			    <% 'along with dropdown,text is also displayed for MBG,Active and Re-Schedule Not Confirmed status
			  
			  If arrAllQueries(57,enrollrowcounter) = "Active" OR arrAllQueries(57,enrollrowcounter) = "MBG" OR arrAllQueries(57,enrollrowcounter) = "Re-Schedule Not Confirmed" Then %>
			  <% If arrAllQueries(57,enrollrowcounter) = "Re-Schedule Not Confirmed" Then %>
			  <% = arrAllQueries(57,enrollrowcounter) %><br />
          ...
          <!--------------------------------------------- Comments ------------------------------------------------------------->
            <form name="comment" action="OnlineEnrolledDb.asp?type=comment" method="post" onsubmit="return Form_Validator3(this)">
              <input type="hidden" name="OnlineEnrollUserId" onChange="myPopup2()" value="<% = arrAllQueries(0,enrollrowcounter) %>">
              <input type="hidden" name="CourseDetails" value="<% = arrAllQueries(6,enrollrowcounter) %>" />
        ...
        <!--------------------------------------------- Enroll Status ------------------------------------------------------------->  
          ...
          <!---********************************************* Enroll Status Form Starts****************************************************--->
            <form name="comment" action="OnlineEnrolledDb.asp?type=enrollStatus" method="post" onsubmit="return Form_Validator4(this)">
              <input type="hidden" name="OnlineEnrollUserId" value="<% =  arrAllQueries(0,enrollrowcounter) %>">
              
              <td><br />
               <% If IsNull(arrAllQueries(65,enrollrowcounter)) OR arrAllQueries(65,enrollrowcounter) = "Active" OR arrAllQueries(65,enrollrowcounter) = "" Then %>
					<% If  IsNull(arrAllQueries(60,enrollrowcounter)) OR  arrAllQueries(60,enrollrowcounter) = "" then%>
                   ...
                   <br /><br />
					<input type="submit" name="type" value="Submit"   class="buttonc"/>
						<% If Not IsNull(arrAllQueries(65,enrollrowcounter)) Then %>
						<% = arrAllQueries(65,enrollrowcounter) %>
						added by
						<% = arrAllQueries(66,enrollrowcounter) %>
						<% End If %>
					<% End If %>
                <% Else %>
                <select name="enrollStatus">
      ...
      <!---********************************************* Enroll Status Form Ends****************************************************--->

          <% allMailSent = arrAllQueries(64,enrollrowcounter) 
	     	 %>
          </tr>
          ...
          <tr>
            <td colspan="19" id="htMap"><% If Not enrollfirstRow = 0 Then %>
              <a href="OnlineCourses.asp?OnlineCourseId=<% = OnlineCourseId %>&enrollPage=previous&enrollRow=<% = enrollfirstRow %>&Course=<% = rqCourse %>&page=<% = iPageCurrent %>"> &lt;&lt;Previous</a>
              <% End If %>
              <% = ((enrollfirstRow + 1) & " - " & (enrolllastRow + 1) & " of " & (enrollnumRows + 1)) %>
              <% If Not enrollnumRows = enrolllastRow Then %>
              <a href="OnlineCourses.asp?OnlineCourseId=<% = OnlineCourseId %>&enrollPage=next&enrollRow=<% = enrolllastRow %>&Course=<% = rqCourse %>&page=<% = iPageCurrent %>"> &gt;&gt;Next</a>
              <% End If %>
            </td>
          </tr>
          <tr>
	       ...
	       Response.Write("All Mails Sent") %>
              <a href="javascript:void(0)" onClick="window.open('sendmails.asp?OnlineCourseId=<% = OnlineCourseId %>')">Send Mails</a>
              <% Else %>
              <a href="javascript:void(0)" onClick="window.open('sendmails.asp?OnlineCourseId=<% = OnlineCourseId %>')">Send Mails</a>
              <!--	 </form>-->
              <% End If %>
