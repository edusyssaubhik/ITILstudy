...
rs.open "SELECT * FROM classroom_feedback Where country = '"&Session("country")&"' AND company  Like '%"& rqCompany &"%'  ORDER BY dateofpassing DESC",Conn

ElseIf rqName <> "" Then

rs.open "SELECT * FROM classroom_feedback Where country = '"&Session("country")&"' AND fname Like '%"& rqName &"%' ORDER BY dateofpassing DESC",Conn

Else 

rs.open "SELECT * FROM classroom_feedback Where country = '"&Session("country")&"' ORDER BY dateofpassing DESC",Conn
...
<tr>
  <td colspan="7"><table border="0" cellpadding="5" cellspacing="0" width="60%">
      <tr>
        ...
        <form action="classroom_feedback.asp" method="post" onsubmit="return Form_Validator3(this)">
          <td>Name:
...
<% 
			    ps = getPaginationString(page, (numRows + 1), 10, 2, "classroom_feedback.asp?action=addEdit")
				  Response.Write(ps)
	           %>
