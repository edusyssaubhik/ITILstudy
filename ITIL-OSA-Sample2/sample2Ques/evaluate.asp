...
Set objRs = ConnObj.Execute("SELECT  QuesNo, SelectedChoice, Marked, Duration from OSAsample2QuesResponse WHERE TestNo='"&TestNo&"' ORDER BY GID")

'Retrive The Correct Answer Table'
Set objRs1 = ConnObj.Execute("SELECT  QuesNo, Ans, KNo from OSAsample2QuesCorrectAns ORDER BY GID")
...
<p align="left">
<!--<UL>
  <LI> <a href="#go1"><B>Summary Test Results</B></A> - Overall Scores and % Correct: You need to get 28 out of the 40 marks (70%) right to pass the foundation exam. <BR>
    <BR>
  </LI>
...
<B>Scores and Justification for each answer:</B>
</P>
...
<% 	
	...
	'Else
		...
		Set objRs2 = ConnObj.Execute("SELECT ChoiceRate FROM Choice_OSAsample2 WHERE QuestionNo = '"&Array(0,i)&"' AND ChoiceNumber = '"&Array(1,i)&"'")
		...
		'Response.Write("SELECT ChoiceRate FROM Choice_OSAsample2 WHERE QuestionNo = '"&Array(0,i)&"' AND ChoiceNumber = '"&Array(1,i)&"'<br>")
...
<BR>
<a name="go1"></a>
<span class="Header">Summary of Results: </span>
<ul><li>
Total Number of Questions: <B>8</B></li>
<li>Total Marks: <b>40</b></li>
<li>Marks Obtained: <b><% = ChoiceRate %></b></li>
<li>Percentage: <b> <%=FormatNumber(ChoiceRate/0.40,2)%></B> % (i.e. <%=ChoiceRate%> / 40 * 100 )</li></ul>
<span class="Header"><A href="/ITIL-OSA-Sample2/sample2Ques/nextsteps.asp">Test Analysis and Next Steps</A></span>
<BR><BR>
...
<%
			...
			Set objRs = connObj.Execute("SELECT TestNo FROM OSAsample2Quesusertable WHERE TestNo = '"&TestNo&"' And KNo1 Is Null")
	 
			If (Not objRs.BOF) And (Not objRs.EOF) Then
			
				OverAllPercentage =correctans/8*100
 	
				Set objRs = connObj.Execute("Update OSAsample2Quesusertable SET EMail = '"&Session("PayerEmail")&"',OverAll = "&OverAllPercentage&" WHERE TestNo = '"&TestNo&"'")
