...
Set objRs = ConnObj.Execute("SELECT  QuesNo, SelectedChoice, Marked, Duration from OSAsample1QuesResponse WHERE TestNo='"&TestNo&"' ORDER BY GID")

'Retrive The Correct Answer Table'
Set objRs1 = ConnObj.Execute("SELECT  QuesNo, Ans, KNo from OSAsample1QuesCorrectAns ORDER BY GID")
...
<% 	
	...
	'Else
		...
		Set objRs2 = ConnObj.Execute("SELECT ChoiceRate FROM Choice_OSAsample1 WHERE QuestionNo = '"&Array(0,i)&"' AND ChoiceNumber = '"&Array(1,i)&"'")
		...
		'Response.Write("SELECT ChoiceRate FROM Choice_OSAsample1 WHERE QuestionNo = '"&Array(0,i)&"' AND ChoiceNumber = '"&Array(1,i)&"'<br>")
...
<span class="Header"><A href="/ITIL-OSA-Sample1/sample1Ques/nextsteps.asp">Test Analysis and Next Steps</A></span>
<BR><BR>
...
<%
			...
			Set objRs = connObj.Execute("SELECT TestNo FROM OSAsample1Quesusertable WHERE TestNo = '"&TestNo&"' And KNo1 Is Null")
	 
			If (Not objRs.BOF) And (Not objRs.EOF) Then
			
				OverAllPercentage =correctans/8*100
 	
				Set objRs = connObj.Execute("Update OSAsample1Quesusertable SET EMail = '"&Session("PayerEmail")&"',OverAll = "&OverAllPercentage&" WHERE TestNo = '"&TestNo&"'")
