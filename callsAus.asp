...
'======================================================='
'Written By : savita'
'Created Date : 25/11/2011'
'Last Updated : 25/11/2011'
'Description :'
'This page is ITILstudy adding Australia calls to the database'
'US BackOffice'
'======================================================='

'Declare the local variables'
Dim strAUSState, objRs, objRs1
...
<table border="1" cellpadding="5" cellspacing="0" align="center" width="45%">
 ...
 <tr>
  <td>
  <form name="addcallsAus" action="addCallsAus.asp" method="post" onsubmit="return Form_Validator(this)">
...
<tr>
	 ...
	 <td>  <select name="state">
	  ...
	  <%
	  'Retrive the states'
		strAUSState = "SELECT state, state_code FROM ITIL_city WHERE country = 'Australia' ORDER BY state"

		objRs.Open strAUSState,ConnObj
	   
	    Do Until objRs.EOF %>
	   <option value="<% = objRs("state") %>"><% = objRs("state") %><%If objRs("state_code") <> "" Then  %>, <% = objRs("state_code") %><%End If%></option>
	   <% objRs.Movenext
	      Loop
		  objRs.Close %>
	  	  </select></td>
